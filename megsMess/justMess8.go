...
func messengerHandler(w http.ResponseWriter, r *http.Request) {
          ...
          "</form>"+
			...
			theValue := r.FormValue("body")
			fmt.Println("text box value " + theValue)
...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if r.Method == "GET" {
        ...
        io.Copy(f, file)
        file.Close()            // not sure if this will works
    } else {
        fmt.Println("Unknown HTTP " + r.Method + "  Method")
    }
    //fmt.Println(fileUp)
    output,err := exec.Command("ipfs", "add", "./uploads/"+fileUp).Output()
    if err != nil {
      fmt.Println(err)
    }

		fileHash = output[6:52]
    fmt.Println(string(fileHash))
    path := "C:/Users/597587/go-ipfs/uploads/"+fileUp
    fmt.Println(path)
    //Remove(path)
}
...
func uploadFileHandler(w http.ResponseWriter, r *http.Request) {
		...
		theLink := "\"http://localhost/ipfs/" + string(fileHash) + "\""
