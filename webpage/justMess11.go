...
func subHandler(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "GET" {
	      ...
	      fmt.Fprintf(w, "subscribed!")
				...
				chatRoom = topic[0]
				fmt.Println("chatroom is " + chatRoom)

				_, err := os.Open(chatRoom + ".txt")
				if err != nil {
						//dont have the file, so create one
						os.Stderr.WriteString(err.Error())
						// WriteStringToFile("messaging.txt", "Start chatting!")
						WriteStringToFile(chatRoom + ".txt", "Start chatting!")
				}
...
func AppendFile(message string) {
	...
	if firstAppend == true {
		 ...
		 WriteStringToFile(chatRoom + ".txt", newOutput)
		 ...
		 firstAppend = false
			 ...
			 WriteStringToFile(chatRoom + ".txt", newMessagingText)
