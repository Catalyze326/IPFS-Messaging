...
func loginHandler(w http.ResponseWriter, r *http.Request){
  r.ParseForm()
  if r.Method == "GET" {
    t, _ := template.ParseFiles("cLogin.html")
    t.Execute(w,nil)
  } else {
    ...
    if attempt == true{                 //if the user login info is correct, page redirects to chat page
      ...
      t, _ := template.ParseFiles("cLogin.html") // if the login info is incorrect, resets
      t.Execute(w,nil)
...
func adminHandler(w http.ResponseWriter, r *http.Request){
  r.ParseForm()
  if r.Method == "GET" {
    t, _ := template.ParseFiles("cAdmin.html")
    t.Execute(w,nil)
  } else {
...
func accountHandler(w http.ResponseWriter, r *http.Request){
  r.ParseForm()
  if r.Method == "GET" {
    t, _ := template.ParseFiles("cSignUp.html")
    t.Execute(w,nil)
  } else {
    t, _ := template.ParseFiles("cSignUp.html")
    t.Execute(w,nil)
...
func subHandler(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "GET" {
				t, _ := template.ParseFiles("cNewSub.html")
				t.Execute(w, nil)
		} else {
				sub(w, r)
	      ...
	      fmt.Fprintf(w, "subscribed!")
				...
				colors := []string{"aqua", "aquamarine", "bisque", "blue", "blueviolet",
												...
												"darkorange", "darkorchid", "darkred", "dodgerblue",
												"forestgreen", "goldenrod", "green", "greenyellow",
												"hotpink", "indianred", "indigo", "lightblue",
												"lightgreen", "maroon", "mediumpurple",
												"mediumspringgreen", "olive", "orangered", "red",
												"purple", "seagreen", "saddlebrown", "yellow"}
				...
				//username = "<font color='darkred'>" + username + "</font>"

				WriteStringToFile("messaging.txt", username + ": has joined <br>")
				//WriteStringToFile(chatRoom + ".txt", username + ": has joined --")

				saveHand = true

				//sendFile(username + " has joined <br>")
...
func saveHandler(w http.ResponseWriter, r *http.Request) {
    ...
    err7 := p.save() //where it saves the new text as p.Body
		...
		message = username + ": " + string(p.Body) + "<br>"
		fmt.Println("message " + message)
...
func messengerHandler(w http.ResponseWriter, r *http.Request) {
	    ...
	    // w.Write(data)
			...
			title := "messaging"
			p, _ := loadPage(title)
			//renderTemplate(w, "cNewMessenger", p)
			...
			if r.Method =="POST"{
				...
				if r.FormValue("test") == "Recording..."{
					...
					} else if r.FormValue("test") == "two"{
					fmt.Println("two success!")
	        ...
	        //fileUp = handler.Filename
				...
				} else if r.FormValue("test") == "Uploading..."{
